...
<div id='processbox'>
    ${h.form(h.url(controller='hello', action='search'), method='GET')}
    ${h.hidden_field(name='limit', value=c.pagesize, checked='checked')}
    ${h.hidden_field(name='offset', value=c.pagenumber, checked='checked')}
    <div class='innerprocessbox'>
        Search for Tag
        <BR>
        Ex: "google"
    </div>
    ${h.text_field(name='tags', id='tags', value='%s', size='128') % c.tags}
    <BR>
    <BR>
    <div class='innerprocessbox'>
        ${h.submit('Process')}
...
<BR>
${h.form(h.url(controller='hello', action='search'), method='GET')}
${h.hidden_field(name='tags', value=c.tags, checked='checked')}
...
<span class="controls-bottom">
    % if c.pagenumber <= 1:
        [ Previous ] -
    % else:
        [ ${h.link_to("Previous", h.url(controller='hello', action='search', limit=c.pagesize, offset=c.pagenumber-1, tags=c.tags), method='GET')} ] -
    % endif
    
    % if c.pagenumber >= c.pageoffset:
        [ Next ]
    % else:
        [ ${h.link_to("Next",h.url(controller='hello', action='search', limit=c.pagesize, offset=c.pagenumber+1, tags=c.tags), method='GET')} ]
    % endif
</span>

<BR><BR>

${h.form(h.url(controller='hello', action='search', method='post'))}
